use crate::models::{BankAccount, Loan};
...
pub async fn total_outstanding_by_user_id(
    db: &PgPool,
    user_id: i64,
...
pub async fn create_loan(
    ...
    let account = lock_account_by_id(&mut transaction, account_id, user_id).await?;
    ...
    let loan = sqlx::query_as::<_, Loan>(
        ...
        VALUES (
            $1, $2, $3, $4, $5, $6, $6, $7, $8, (CURRENT_DATE + make_interval(months => 1))::DATE, 'active'
        )
...
pub async fn pay_loan(
    ...
    let loan = lock_loan(&mut transaction, user_id, loan_id).await?;
    let account = lock_account_by_id(&mut transaction, loan.account_id, user_id).await?;

    let amount_to_pay = payment_cents
        .unwrap_or(loan.monthly_payment_cents)
        .min(loan.remaining_cents);
...
//user can only boorow if any loan less tham 3 months overdue
